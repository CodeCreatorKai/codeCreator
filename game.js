...
// --- Key state tracking to prevent super-speed ---
...
function handleMovement() {
  if (keys.ArrowUp) { stickman.y -= 50; score++; if (stickman.y <= 0) advanceLevel(); keys.ArrowUp = false; }
  if (keys.ArrowDown) { stickman.y += 50; keys.ArrowDown = false; }
  if (keys.ArrowLeft) { stickman.x -= 30; keys.ArrowLeft = false; }
  if (keys.ArrowRight) { stickman.x += 30; keys.ArrowRight = false; }
...
function updateObstacles() { 
  obstacles.forEach((o,i)=>{ o.x+=o.dir*o.speed; if(o.x<-o.w||o.x>canvas.width+o.w) obstacles.splice(i,1); }); 
}
...
function checkCollision() {
  ...
  for(let i=0;i<powerUps.length;i++){
    ...
    if(stickman.x<p.x+p.size && stickman.x+stickman.size>p.x && stickman.y<p.y+p.size && stickman.y+stickman.size>p.y){
      score+=10; stickman.invincible=true; stickman.invincibleTimer=300;
...
function drawStickman(){
  ...
  ctx.beginPath(); ctx.moveTo(cx,cy-2); ctx.lineTo(cx,cy+15); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx-10,cy+2); ctx.lineTo(cx+10,cy+2); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(cx,cy+15); ctx.lineTo(cx-8,cy+30); ctx.moveTo(cx,cy+15); ctx.lineTo(cx+8,cy+30); ctx.stroke();
}

// --- Draw everything ---
function draw(){
  ctx.clearRect(0,0,canvas.width,canvas.height);
  for(let i=0;i<10;i++){ ctx.fillStyle=i%2===0?"#2a2a2a":"#3a3a3a"; ctx.fillRect(0,i*60,canvas.width,60); }
